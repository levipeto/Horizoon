...
<div class="w-full adaptable:block adaptable:mt-32 adaptable:overflow-hidden 
adaptable:overflow-y-scroll flex h-screen mobile:block mobile:space-x-0 space-x-2 mt-24 p-4">
  <div class="w-1/2 adaptable:w-full mobile:w-full pt-8 pl-8 adaptable:pl-0">
      <div class="h-full w-full block pl-4 adaptable:pl-0">
        @foreach ($carts as $cart)
        <div class="w-full adaptable:max-w-xl adaptable:mx-auto adaptable:block adaptable:p-0 flex space-x-2 p-2">
          <div class="w-1/3 adaptable:mx-auto">
            <img class="object-center object-cover pt-4" name='image' src="{{ Storage::url($cart->image) }}" width="140" height="140">
          </div>
        ...
        <hr>
        @endforeach
        <form class="mt-12" action="{{ route('clear') }}" method="POST">
         @csrf
  ...
  {{-- Total --}}
  <div class="w-1/2 h-full mobile:w-full adaptable:w-full">
    <form action="{{route('stripe_payment', $total)}}" method="GET">
    @csrf
    <div class="bg-white w-80 mx-auto rounded-sm shadow-2xl p-4 mt-12 adaptable:w-full mobile:w-full">
      <div class="p-2">
